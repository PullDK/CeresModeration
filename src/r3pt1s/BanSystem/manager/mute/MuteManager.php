...
class MuteManager {
    ...
    public function mutePlayer(Player|string $player, Player|string $moderator, int $id) {
        ...
        $mutedAt = (new \DateTime("now"))->format("d-m-Y H:i:s");
        ...
        NotifyManager::sendNotify(
            BanSystem::getPrefix() . "§e" . $player . " §7mutado por §c" . $moderator . "§7!\n" .
    ...
    public function unmutePlayer(Player|string $player, Player|string $moderator = "automatic") {
        ...
        NotifyManager::sendNotify(BanSystem::getPrefix() . "§e" . $player . " §7foi desmutado por §c" . $moderator . "§7!");
    }
    ...
    public function editMute(Player|string $player, string $time, string $type = "add", ?string &$errorMessage = null): bool {
        ...
        if ($info !== false) {
            if ($info["Time"] !== "-1") {
                ...
                CurrentProvider::get()->editMute($player, $newTime->format("d-m-Y H:i:s"));
                return true;
            } else $errorMessage = "§7O mute do jogador §e" . $player . " §7foi editado!";
        } else $errorMessage = "§7O jogador §e" . $player . " §7não foi mutado!";
        return false;
    }
