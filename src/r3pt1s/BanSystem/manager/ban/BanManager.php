...
class BanManager {
    ...
    public function banPlayer(Player|string $player, Player|string $moderator, int $id) {
        ...
        $bannedAt = (new \DateTime("now"))->format("d-m-Y H:i:s");
        $time = ($idData["duration"] == "-1" ? "-1" : Utils::convertStringToDateFormat($idData["duration"])->format("d-m-Y H:i:s"));
        ...
        NotifyManager::sendNotify(
            BanSystem::getPrefix() . "§e" . $player . " §7Banido por §c" . $moderator . "§7!\n" .
            BanSystem::getPrefix() . "§7Motivo: §e" . $idData["reason"] . "\n" .
            BanSystem::getPrefix() . "§7Tempo: §e" . str_replace("-1", "PERMANENTLY", $time)
        );
    }
    ...
    public function tempBanPlayer(Player|string $player, Player|string $moderator, string $reason, string $duration) {
        ...
        $bannedAt = (new \DateTime("now"))->format("d-m-Y H:i:s");
        $time = ($duration== "-1" ? "-1" : Utils::convertStringToDateFormat($duration)->format("d-m-Y H:i:s"));
        ...
        NotifyManager::sendNotify(
            BanSystem::getPrefix() . "§e" . $player . " §7banido por §c" . $moderator . "§7!\n" .
            BanSystem::getPrefix() . "§7Motivo: §e" . $reason . "\n" .
            BanSystem::getPrefix() . "§7Tempo: §e" . str_replace("-1", "PERMANENTLY", $time)
        );
    }
    ...
    public function unbanPlayer(Player|string $player, Player|string $moderator = "automatic") {
        ...
        else NotifyManager::sendNotify(BanSystem::getPrefix() . "§e" . $player . " §7desbanido por §c" . $moderator . "§7!");
    }
    ...
    public function editBan(Player|string $player, string $time, string $type = "add", ?string &$errorMessage = null): bool {
        ...
        if ($info !== false) {
            if ($info["Time"] !== "-1") {
                ...
                CurrentProvider::get()->editBan($player, $newTime->format("d-m-Y H:i:s"));
                return true;
            } else $errorMessage = "§7O ban do jogador §e" . $player . " §7foi editado!";
        } else $errorMessage = "§7O jogador §e" . $player . " §7não foi banido!";
        return false;
    }
